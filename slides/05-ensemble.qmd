...
|---|---|
| **Core** | Explain voting, diversity, bagging, random forests, boosting, and weak learners |
| **Practice** | Compute majority vote, weighted vote |
...
- Partition the classes into groups
...
- A model with high **bias** is too restricted to represent the true boundary (a very shallow decision tree may have high bias)
- A model with high **variance** changes too much when the training set changes (a 1-NN classifier may have low bias but high variance)
- A model affected by **noise** cannot remove ambiguity already present in the data (mislabeled records can hurt any classifier)


# Bagging and boosting

![Both methods combine multiple models, but they create diversity in different ways. Bagging = stabilize; Boosting = correct](img/ensemble/bagging-vs-boosting.png)

- *Bagging* mainly reduces **variance**
- *Boosting* reduces **bias** and often variance, but may become sensitive to noise (e.g., AdaBoost, Gradient Boosting, and XGBoost)
...
# Bootstrap sampling {visibility="hidden"}
...
Consider a one-level binary decision tree, also called a **decision stump** where $s$ is the split point learn by the stump:

$$
x \le s \rightarrow ❌,\qquad x>s \rightarrow ✅
$$

A single stump cannot perfectly model this pattern. *Why*?
...
$$
x \le 0.3 \rightarrow ✅,\qquad x>0.3 \rightarrow ❌
$$

Other samples emphasize the right boundary:

$$
x \le 0.7 \rightarrow ❌,\qquad x>0.7 \rightarrow ✅
$$
...
- **Features**: each split considers only a random subset of features
    ...
    - Typical choice for classification: $d' \approx \sqrt{d}$, where $d$ is the number of features and $d'$ is the number considered at each split.
:::
:::{.column width="40%"}
...
# Weak learners {visibility="hidden"}
...
# AdaBoost in the original paper {visibility="hidden"}
...
# AdaBoost notation {visibility="hidden"}
...
# Weighted error {visibility="hidden"}
...
# Classifier weight {visibility="hidden"}
...
# Weight update {visibility="hidden"}
...
# Final AdaBoost classifier {visibility="hidden"}
...
# AdaBoost algorithm {visibility="hidden"}
...
# AdaBoost algorithm in pseudocode {visibility="hidden"}
...
# Worked mini-example {visibility="hidden"}
...
# AdaBoost with decision stumps {visibility="hidden"}
...
# Geometric intuition {visibility="hidden"}
...
Adaboos [@freund1997decision], XGBoost [@chen2016xgboost], and LightGBM [@ke2017lightgbm] could be target algorithms for your assignment!

# Summary

The key question is: **how do we create useful diversity among classifiers?**
