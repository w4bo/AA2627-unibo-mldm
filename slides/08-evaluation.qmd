...
- **Precision** ($Precision = \frac{TP}{TP + FP}$): among predicted positives, how many are actually positive?
    ...
    - The model is "punished" for guessing a dog when there is no dog
    - Better to guess a dog, when you are very sure that you see a dog
- **Recall** ($Recall = \frac{TP}{TP + FN}$): among actual positives, how many did the model detect?
    - High recall means few false negatives.
    - Better safe than sorry!
    - Be optimistic and guess dog even if you are not sure

:::
:::{.column width=30%}

![Performance metrics](https://thumb.wikimedia.org/wikipedia/commons/thumb/b/bc/PrecisionrecallDogExample.svg/500px-PrecisionrecallDogExample.svg.png?utm_source=en.wikipedia.org&utm_campaign=imageinfo&utm_content=thumbnail)
...
- Prefer higher precision (few false positives) when a positive prediction triggers a disruptive or irreversible action.
...
- Prefer higher (few false negatives) recall when missing a positive case creates danger or a major loss.
...
::::{.columns}
:::{.column width=50%}
$Precision = 1$ if no FP: every predicted positive is actually positive.

![$Precision = 1$](./img/dt/image25.png)
:::
:::{.column width=50%}
$Recall = 1$ if no FN: every actual positive is detected.
...
# What about the evaluation of model hyperparameters?
...
:::
:::{.column width=50%}
![Cross validation](./img/cv/grid_search_cross_validation.png)
:::
::::
...
- Cross-validation creates $k$ different classifiers
...
![Cross validation](./img/cv/grid_search_workflow.png)
