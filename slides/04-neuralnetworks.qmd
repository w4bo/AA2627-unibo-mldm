...
|---|---|
| **Core** | Explain artificial neurons, layers, activation functions, loss, gradient descent, and backpropagation intuition |
| **Practice** | Compute a small forward pass and interpret learning curves |
| **Extension** | Detailed numeric backpropagation, hardware choices, and frontier-model training costs |

# Project lifecycle checkpoint

| Project question | Neural-network answer |
|---|---|
| Business question | Is the prediction problem complex enough to need learned representations? |
| Data requirements | Enough representative data to train and validate many parameters |
| Preprocessing | Normalize inputs, encode outputs, and keep transformations inside the pipeline |
| Modeling | Choose architecture, loss, optimizer, and regularization |
| Evaluation | Monitor training and validation curves; test the selected workflow once |
| Failure modes | Underfitting, overfitting, unstable optimization, biased data, and excessive complexity |
| Assignment angle | Justify why a neural network is needed instead of a simpler baseline |
...
| Project question | Neural-network answer |
|---|---|
| Business question | Does the task require nonlinear representation learning? |
| Data requirements | Data volume, label quality, and deployment representativeness matter strongly |
| Preprocessing | Input scaling and output encoding affect training behavior |
| Modeling | Architecture, loss, optimizer, and regularization work together |
| Evaluation | Learning curves should explain both performance and training failures |
| Failure modes | A powerful model can hide leakage, bias, or unnecessary complexity |
| Assignment angle | Compare against a simpler model before claiming the added complexity is useful |
