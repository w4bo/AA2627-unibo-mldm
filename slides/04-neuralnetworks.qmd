...
# Recalling the Machine Learning pipeline {visibility="hidden"}

We now focus on the model, the "heart" of the AI in our system.

![Data pipeline](./img/datapreprocessing/pipeline.svg)

- By *model*, we mean the "mechanism" by which input data are transformed into outputs
...
![Mathematical Symbol Fight](./img/remote/xkcd-mathematical-symbol-fight.png)
...
The scheme about the neuron was introduced by [@mcculloch1943logical]
...
- Inputs are *merged* with a weighted sum function (plus a *bias*)
- An *activation function* is used to fitler the output:
  - Perceptron uses the step function $f(h)=\begin{cases}1,&h\geq 0\\0,&h<0\end{cases}$
  - $h = \sum_i w_i x_i +b$
...
# Prediction: will I pass the exam?

![Example of neuron (weights and bias are given)](./img/neuralnetworks/slide54.png)

$y' = \begin{cases}1, ~if~ 0.3\cdot x_1 + 0.8 \cdot x_2 + 0.5 \cdot x_3 \ge 3\\0, otherwise\end{cases}$

# Training: how can we set the weights and bias to implement the following operators?
...
# ANN and DNN training: intuition
...
See also [interactive gradient descent](https://blog.skz.dev/gradient-descent)
:::
::: {.column width="60%"}

1. This surface is the solution space of the *loss function* (low values, low error)
2. *Starting point*: output of the NN with initial (random) weights
...
# ANN and DNN training {visibility="hidden"}
...
**Cross Entropy** is the distance between what the model believes the output distribution should be and what the original distribution is
...
# Cross Entropy + Softmax {visibility="hidden"}
...
How to minimize the loss? *By adjusting (changing) the weights and the bias of every neuron*
...
**Gradient descent** is the rule used to improve the network parameters

- Intuition: if increasing a weight increases the loss, gradient descent decreases that weight.
- The **gradient** tells how the loss changes if each weight/parameter changes a little
...
2. Use the current parameters to compute predictions on the **whole training data**
...
- Smooth convergence
- Not recommended for huge training datasets. *Why*?
...
# Gradient descent {visibility="hidden"}
...
# Gradient descent for a single neuron: the delta rule
...
The **chain rule** tells us how to differentiate composite functions: functions built by applying one transformation after another.
...
The following slides (toy example) show the math behind ANN

- They are examples of how ANN are trained
- ... but you can skip the math!
...
Before training, the network predicts too low for $(x_1=1,x_2=1)$.
...
- With threshold $0.5$, the network predicts $\hat{y}=0$ but the correct target is $y=1$
...
- Since the gradients are negative, subtracting them increases the weights and biases.
- Assume $\eta=10$. 
...
The previous update fixes one example.

- After training on all four AND rows, one possible set of weights is:
...
The learning rate is important. *Why*? See [this demo](https://blog.skz.dev/gradient-descent)
...
- 2 plots (usually overlapped) of learning curves:
  - One for the training
  - One for the validation
- They help to diagnose whether the model has over-learned (*overfitting*), under-learned (*underfitting*), or suitably fits the training dataset.
