...
During the training phase, we learn the probabilistic relationships $P(C|\boldsymbol{A})$ for each combination of values assumed by $\boldsymbol{A}$ and $C$

Knowing these probabilities, a test record can be classified by finding the class label $c$ that maximizes the posterior probability $P(\boldsymbol{C}=c|A_1=a_1,..., A_n=a_n)$
...
- Bayes' theorem is useful here because it allows the posterior probability $P(C|\boldsymbol{A})$ to be expressed in terms of $P(\boldsymbol{A}|C)$, $P(C)$, and $P(\boldsymbol{A})$

$P(C|A_1, A_2, ..., A_n)=\frac{P(A_1, A_2, ..., A_n|C)\cdot P(C)}{P(A_1, A_2, ..., A_n)}$

- Since $P(\boldsymbol{A})$ is constant in this formula, the problem of maximizing the posterior probability is equivalent to choosing the value of $C$ that maximizes
...
Because of conditional independence, we can write:

$P(X,Y|Z)=\frac{P(X,Y,Z)}{P(Z)}=\frac{P(X,Y,Z)}{P(Y,Z)} \cdot=\frac{P(Y,Z)}{P(Z)}=P(X|Y,Z)\cdot P(Y|Z)=P(X|Z) \cdot P(Y|Z)$

# Naïve Bayes (cont.)

The assumption of independence between attributes $A_i$ when class $C$ is known allows us to rewrite:
