...
* $m_i = \text{\# objects in cluster } i$
* $m_{ij} = \text{\# objects of class } j \text{ in cluster } i$
* $p_{ij} = \frac{m_{ij}}{m_i}$ is the fraction of cluster $i$ that belongs to class $j$

**Entropy** measures how mixed the classes are inside each cluster.

- $e_i = - \sum_{j=1}^{L} p_{ij} \cdot log_2(p_{ij})$
- $e = \sum_{i=1}^{K} \frac{m_i}{m} \cdot e_i$

Lower entropy means cleaner clusters.

**Purity** keeps only the majority class in each cluster.

- $p_i = max_j(p_{ij})$
- $p = \sum_{i=1}^{K} \frac{m_i}{m} \cdot p_i$

Higher purity means cleaner clusters.

$L=$ #classes, $K=$ #clusters, $m=$ #objects
