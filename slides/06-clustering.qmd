...
| **Extension** | Hopkins statistic, cophenetic distance, and external validity measures |

# Project lifecycle checkpoint

| Project question | Clustering answer |
|---|---|
| Business question | Would discovered groups support different actions or explanations? |
| Data requirements | Features must encode meaningful similarity without relying on labels |
| Preprocessing | Scale numeric features and justify distance or similarity |
| Modeling | Choose cluster notion, algorithm, parameters, and representation |
| Evaluation | Validate stability, tendency, and domain usefulness |
| Failure modes | Random structure, arbitrary $K$, outliers, variable density, and misleading visual separation |
| Assignment angle | Defend why the clusters are meaningful for the domain, not only mathematically present |
...
| Project question | Clustering answer |
|---|---|
| Business question | What action becomes possible if these groups are real? |
| Data requirements | Similarity must reflect the domain, not just convenient columns |
| Preprocessing | Scaling and representation often determine the result |
| Modeling | Algorithm choice encodes a definition of "cluster" |
| Evaluation | Stability and domain validation matter more than a pretty plot |
| Failure modes | Clusters can be artifacts of parameters, preprocessing, or randomness |
| Assignment angle | Show how cluster interpretation changes decisions |
