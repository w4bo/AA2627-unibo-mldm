...
- **Execute** the main steps of k-means and DBSCAN
...
|---|---|
| **Core** | Explain cluster ambiguity, k-means, hierarchical clustering, DBSCAN, and cluster validity |
| **Practice** | Run one k-means iteration and compare algorithms on shape, density, noise, and interpretability |

# Motivating problem: discover customer segments

A mall has customer behavior data but no segment labels.
...
The business question is "what are the different customer segments?"

- Can we design different offers for different behaviors?
- How can we identify customers who don't fit into any existing segment?

# Suppose we observe a few mall customers with *no segment label*
...
mall_customers = pd.DataFrame({
    "customer_id": [f"C{i:02d}" for i in range(1, 13)],
    "age": [22, 24, 27, 29, 46, 49, 52, 55, 31, 34, 36, 39],
    "annual_income_k": [25, 28, 30, 33, 70, 73, 76, 79, 82, 86, 90, 94],
    "spending_score": [78, 82, 74, 88, 24, 31, 28, 35, 76, 84, 81, 89],
})

mall_customers
```

# Clustering finds useful groups (segments) in unlabeled data
...
X_mall = scaler.fit_transform(mall_customers[features])

kmeans = KMeans(n_clusters=3, n_init=20, random_state=42)
mall_clustered = mall_customers.assign(cluster=kmeans.fit_predict(X_mall))

mall_clustered[["customer_id", "cluster"] + features].sort_values("cluster")
```

# Group profiles can be summarized with descriptive statistics
...
cluster_summary["interpretation"] = [
    ...
    if row.avg_income_k >= 70 and row.avg_spending >= 70
    else "High income, low spending"
    if row.avg_income_k >= 70
    else "Lower income, high spending"
...
2. **How does the algorithm define clusters?** Centroids or density reachability
...
The definition does **not** determine the right similarity, number of clusters, or cluster shape.

- *Those are modeling choices.*

# Cluster notion can be ambiguous: how many clusters?
...
# Types of *cluster*: Well-Separated {visibility="hidden"}
...
* A cluster is a set of points such that a point in the cluster is closer to the "center" of the cluster, rather than the centers of other clusters
* Cluster center is called *centroid* if it is computed as the average of all the cluster's points
    - ... or *medoid* if it is computed as the most representative point in the cluster

![4 center-based clusters](img/clustering/Immagine15.svg)

# Types of *cluster*: Contiguity-Based {visibility="hidden"}
...
# Types of *cluster*: Conceptual cluster {visibility="hidden"}
...
kmeans(k, points):
    ...
    repeat
        ...
        update each centroid (mean of cluster points)
...
natural_centers = np.array([[-4, 0], [-1, 0], [2, 0], [5, 0]])
points_per_cluster = 35

X_optimality = np.vstack([
    rng.normal(loc=center, scale=0.55, size=(points_per_cluster, 2))
    for center in natural_centers
])
natural_labels = np.repeat(np.arange(len(natural_centers)), points_per_cluster)

good_initial_centroids = natural_centers.copy()
optimal_kmeans = KMeans(
    n_clusters=4, init=good_initial_centroids, n_init=1, random_state=1
)
optimal_labels = optimal_kmeans.fit_predict(X_optimality)
...
suboptimal_kmeans = KMeans(
    n_clusters=4, init=bad_initial_centroids, n_init=1, random_state=1
)
suboptimal_labels = suboptimal_kmeans.fit_predict(X_optimality)

fig, axes = plt.subplots(1, 3, figsize=(12, 4), sharex=True, sharey=True)
plots = [
    (natural_labels, natural_centers, None, "Original points (natural clusters)"),
    (
        optimal_labels,
        optimal_kmeans.cluster_centers_,
        good_initial_centroids,
        f"Lucky initialization (SSE = {optimal_kmeans.inertia_:.1f})",
    ),
    (
        suboptimal_labels,
        suboptimal_kmeans.cluster_centers_,
        bad_initial_centroids,
        f"Unlucky initialization (SSE = {suboptimal_kmeans.inertia_:.1f})",
    ),
]

for i, (ax, (labels, centers, initial_centroids, title)) in enumerate(zip(axes, plots)):
    ax.scatter(
        X_optimality[:, 0], X_optimality[:, 1],
        c=labels, cmap="tab10", s=26, alpha=0.8, edgecolors="none"
    )
    if i > 0:
        ax.scatter(
            centers[:, 0], centers[:, 1],
            c="black", marker="x", s=115, linewidths=2.3
        )
    if initial_centroids is not None:
        ax.scatter(
            initial_centroids[:, 0], initial_centroids[:, 1],
            c="crimson", marker="+", s=150, linewidths=2.6
        )
    ax.set_title(title)
    ax.set_xticks([])
    ax.set_yticks([])
...
# Random starts can lead to different final configurations.
...
random_initial_centroids = X_optimality[
    np.random.default_rng(3).choice(len(X_optimality), size=4, replace=False)
]
random_suboptimal_kmeans = KMeans(
    n_clusters=4, init=random_initial_centroids, n_init=1, random_state=1
)
random_suboptimal_labels = random_suboptimal_kmeans.fit_predict(X_optimality)
...
plots = [
    ...
    (
        random_suboptimal_labels,
        random_suboptimal_kmeans.cluster_centers_,
        random_initial_centroids,
        f"Another unlucky initialization (SSE = {random_suboptimal_kmeans.inertia_:.1f})",
    ),
]
...
def kmeans_iterations(X, k, random_state, n_iterations):
    ...
    for _ in range(n_iterations):
        ...
        for cluster in range(k):
            ...
            if len(cluster_points) > 0:
                next_centroids[cluster] = cluster_points.mean(axis=0)
        centroids = next_centroids

    return history

history = kmeans_iterations(
    X_optimality, k=4, random_state=1, n_iterations=6
)
starting_centroids = history[0][1]

fig, axes = plt.subplots(2, 3, figsize=(12, 6), sharex=True, sharey=True)

for iteration, (ax, (labels, centroids)) in enumerate(zip(axes.ravel(), history), start=1):
    ax.scatter(
        X_optimality[:, 0], X_optimality[:, 1],
        c=labels, cmap="tab10", s=22, alpha=0.75, edgecolors="none"
    )
    ax.scatter(
        centroids[:, 0], centroids[:, 1],
        c="black", marker="x", s=110, linewidths=2.2
    )
    ax.scatter(
        starting_centroids[:, 0], starting_centroids[:, 1],
        c="crimson", marker="+", s=140, linewidths=2.5
    )
...
* The algorithm can converge to a suboptimal solution
...
* $n$ = number of points, $K$ = number of clusters, $I$ = number of iterations, $d$ = dimensionality of the data

# Evaluating k-means clusters

The most common measure is the Sum of Squared Error

* $SSE = \sum_{i=1}^K\sum_{x \in C_i} dist(x, m_i)^2$
* For each point $x$, the error is the distance to the nearest cluster centroid $m_i$ of cluster $C_i$.

One easy way to reduce SSE is to increase  $K$, the number of clusters.

* A good clustering with a smaller $K$ can have a lower SSE than a poor clustering with a higher $K$
* It is not necessarily true that the final configuration is the one that has the minimum value of SSE. *Why*?
...
* $P=\frac{\text{\# ways of choosing a centroid by cluster}}{\text{\# ways of choosing K centroids}}=\frac{(n)^K \cdot K!}{(n \cdot K)^K}=\frac{K!}{K^K}$
    * If $K$ = 10, probability is $\frac{10!}{10^{10}} \approx 0.00036$
...
# Solution to the problems induced by the choice of initial centroids {visibility="hidden"}
...
* Choose an item of the cluster with the highest SSE.
    * Normally, this causes the cluster to split into two clusters that include the closest elements.
...
The goodness of clustering can be negatively influenced by the presence of outliers that tend to "shift" the cluster centroids
...
# K-means limits: non-globular clusters
...
# Possible solution: use a higher $k$ value
...
Execute k-means several times with increasing values for k (higher k, lower SSE)
...
# Hierarchical Clustering {visibility="hidden"}
...
# Hierarchical Clustering: pros and cons {visibility="hidden"}

Pros:

* The number of clusters must not be defined a priori
...
# Hierarchical Clustering approaches {visibility="hidden"}
...
# Hierarchical Clustering algorithms {visibility="hidden"}
...
# Hierarchical Clustering flow {visibility="hidden"}
...
# Merging step {visibility="hidden"}
...
# Inter-Cluster Distances {visibility="hidden"}
...
# Hierarchical clustering: MIN {visibility="hidden"}
...
# MIN: pros and cons {visibility="hidden"}
...
# Hierarchical clustering: MAX {visibility="hidden"}
...
# MAX: pros and cons {visibility="hidden"}
...
# Hierarchical clustering: Group Average {visibility="hidden"}
...
# Hierarchical clustering: Ward's Method {visibility="hidden"}
...
* It can be used to initialize k-means because it helps identify the correct value of $k$ and an initial subdivision of the points
    * Please note that, because previous choices cannot be undone, the solutions found by hierarchical methods are often suboptimal.

# Hierarchical clustering: Comparison {visibility="hidden"}

![Comparison](img/clustering/Immagine44.svg)

# Hierarchical clustering: computational complexity {visibility="hidden"}
...
# Hierarchical clustering: exercise {visibility="hidden"}
...
* Data with high dimensionality
  ...
  * Does not handle datasets with variable density
...
::::{.columns}
:::{.column width="50%"}
Choose $MinPts$ first, then use it to estimate $Eps$.

* $MinPts$ defines how dense a region must be before DBSCAN treats it as a cluster
* A common starting point is $MinPts \geq d + 1$, where $d$ is the number of dimensions
* In practice, larger values make DBSCAN more conservative and more robust to noise
* For two-dimensional datasets, $MinPts = 4$ is a common default

Once $MinPts$ is fixed, choose $Eps$ with a $k$-distance plot:

* For each point, compute the distance to its $k$-th nearest neighbor, with $k = MinPts$
* Sort these distances from smallest to largest
* Points inside dense clusters have similar $k$-neighbor distances
* Noise and sparse points have much larger distances
* Choose $Eps$ near the "elbow" of the curve, where distances start increasing sharply
:::
:::{.column width="50%"}
...
For supervised classification techniques, there are several measures to evaluate the validity of the results
...
Cluster Cohesion: Measures how closely related the objects in a cluster are (Within cluster Sum of Squares or SSE)

* $SSE = WSS = \sum_{i=1}\sum_{x \in C_i} dist(x, m_i)^2$

Cluster Separation: Measure how distinct or well-separated a cluster is from other clusters (Between cluster Sum of Squares)

* $BSS = \sum_{i=1} |C_i| \cdot dist(m_i, m)^2$

# Internal Measures: Cohesion and Separation

Cohesion and separation can be calculated for graph-based representations...
...
... as well as for representations based on centroids
...
# Measuring Cluster Validity Via Correlation {visibility="hidden"}
...
Correlation between the incidence matrix and the proximity matrix for the result of the k-means algorithm on two different datasets.

* The correlation is negative because small distances in the proximity matrix correspond to large values (1) in the incidence matrix
...
If the data are uniformly distributed, the matrix is more evenly shaded.
...
Associate similarity matrices with the dataset
...
# Cophenetic distance {visibility="hidden"}
...
Internal measures often return measurements that must be interpreted: _Is a value of 10 good or bad?_
...
* Therefore, the idea is to compare the value of the measure with that obtained from random data.
  * If the value of the measure obtained on the data is unlikely for random data, then the clustering is valid
...
* Some types of clusters may not be identified by the chosen algorithm
...
* Applicable mainly to low-dimensional data in Euclidean spaces

# Clustering Tendency: Hopkins statistic

Hopkins asks a simple question:

> Do real data points look more "near each other" than points placed randomly in the same space?

Procedure:

1. Sample some real points from the dataset
2. Generate the same number of random points in the data space
3. For both groups, measure the distance to the nearest real neighbor
4. Compare the two distance sums

$H = \frac{\sum_{i=1}^{m} u_i}{\sum_{i=1}^{m} u_i + \sum_{i=1}^{m} w_i}$

- $u_i$: distance from a random point to the nearest real point
- $w_i$: distance from a real sampled point to the nearest other real point

# Clustering Tendency: interpreting Hopkins

If the data contain clusters, a sampled real point is usually close to another real point, while a random point may fall in an empty region.

| Value | Intuition | Interpretation |
|---|---|---|
| $H \approx 0.5$ | Random and real points have similar nearest-neighbor distances | No strong clustering tendency |
| $H \rightarrow 1$ | Real points are much closer to each other than random points are | Clustered structure |
| $H \rightarrow 0$ | Random points are closer to real points than sampled real points are | Regular/evenly spaced structure |
...
Assume external class labels are available.

$L=$ #classes, $K=$ #clusters, $m=$ #objects, given a cluster $c_i$:

* $m_i = |c_i|$
* $m_{ij} = \text{\# objects of class } j \in c_i$
* $p_{ij} = \frac{m_{ij}}{m_i}$ is the fraction of cluster $c_i$ that belongs to class $j$

**Entropy** measures how mixed the classes are inside each cluster: lower entropy means cleaner clusters.

- $e_i = - \sum_{j=1}^{L} p_{ij} \cdot log_2(p_{ij})$
- $e = \sum_{i=1}^{K} \frac{m_i}{m} \cdot e_i$

**Purity** keeps only the majority class in each cluster: higher purity means cleaner clusters.
