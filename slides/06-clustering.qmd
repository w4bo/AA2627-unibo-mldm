...
* The centroid $m_i$ is the average of the cluster points, $x$ is a data point in cluster $C_i$.
...
Clusters may change in an iteration.

* So every time the configuration changes, it must lead to a state never visited before
...
The algorithm must stop due to the unavailability of further configurations to visit.
