...
![Example of pattern](img/00-ml/loan.svg)

Pattern: `IF income < k THEN reject ELSE accept`
...
:::{.fragment}
- We look for co-occurences between surnames and regions in people attributes
:::

Group documents based on context information (eg "Amazon rainforest", "Amazon.com")
...
:::: {.columns}
::: {.column width="50%"}
"Machine learning" is distinct from "data mining"
...
* Each record is composed by a set of _attributes_ (aka features), where one ot them represents the _class_ (aka label) of the record.
...
1. The original dataset is split into two subsets: training set and test set.
    * A _training set_ is used to learn the model.
    * A  _test set_ is used to determine the model accuracy and its generalization capabilities on unseen data.
...
- At each step, mergee the two most similar clusters based on a chosen distance metric
