...
- Never use the test set for training/tuning the model

Disadvantages:

- It works with a reduced set of training.
- The result depends on the composition of the training set and test set. *Why*?
...
::::{.columns}
:::{.column width=60%}
Decision trees solve classification problems through sequences of simpler questions.
...
- Let $D_t=\{x_1, ..., x_k\}$ be the records of the training set corresponding to $node_t$ and $y_t = \{y_1, ..., y_k\}$ the possible labels
- Apply the current procedure for each subset recursively, if $D_t$
...
:::
:::{.column width=50%}
*Binary split*: creates two partitions only.

- The attribute value that optimally splits the dataset must be found.
...
::::{.columns}
:::{.column width=50%}
*N-ary split*: The split condition is expressed as a Boolean test that results in multiple ranges of values.

- The algorithm must consider the entire possible range of values as possible split points
...
:::
:::{.column width=50%}
*Binary split*: The split condition is expressed as a binary comparison test.

- The algorithm must consider all values as possible split points
...
# Computing Gini for *categorical* attributes {visibility="hidden"}
...
However, maximizing raw $Gain$ can favor attributes with many distinct values. *Why*?
...
Consider these churn-prediction training records:

| `customer_id` | `auto_pay` | `monthly_watch_hours` | `tickets` | `churn` |
| :- | :-: | :-: | :-: | :-: |
| C-1048 | no | 3 | 3 | ✅ |
| C-1172 | no | 5 | 2 | ✅ |
| C-1325 | no | 6 | 4 | ✅ |
...
At the root: 4 `yes` and 5 `no`, so $Gini(root)=0.494$.

# Gain-based split

A unique identifier can look like the best possible split under raw Gini gain:

| Candidate split | Children | Weighted Gini | Gini gain |
| :- | :- | :-: | :-: |
| `customer_id` | 9 one-record children, each pure | **0.000** | **0.494** |

For `customer_id`, every child has either 1 churner or 1 non-churner:

$$
Gini(customer\_id)=\sum_{i=1}^{9}\frac{1}{9}\cdot 0=0
$$

The score is perfect on training data, but the split only memorizes customer identifiers.

# Split based on $SplitInfo$

To avoid the problem of spraying records into many tiny branches, we can maximize the $GainRatio$.
...
See [Evaluation of classifiers](08-evaluation)
...
A node cannot be split further if:
...
Instances in the new leaf can be tagged based on the label that appears most frequently in the sub-trees

Post-pruning is more effective but involves more computational cost.

- It is based on the evidence of the result of a complete tree.

# Notes on overfitting {visibility="hidden"}

Overfitting results in more complex decision-making trees than necessary

The classification error on the training set does not provide accurate estimates of tree behavior on unknown records.

This requires new techniques to estimate generalization error.

# Estimating tree generalization error {visibility="hidden"}
...
# Pessimistic approach {visibility="hidden"}
...
# Post-pruning example {visibility="hidden"}
...
- No attribute produces a positive $GainRatio$
...
Use a decision tree when you need an interpretable model expressed as rules.
